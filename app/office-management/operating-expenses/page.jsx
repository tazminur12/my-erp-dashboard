...
const OperatingExpenses = () => {
  ...
  const handleCategoryClick = (categoryId) => {
    router.push(`/office-management/operating-expenses/${categoryId}`);
  };

  const handleAddCategory = () => {
    router.push('/office-management/operating-expenses/add');
  };
  ...
  const handleDeleteCategory = async (e, categoryId, categoryName) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`/api/operating-expenses/categories/${categoryId}`, {
          method: 'DELETE',
        });
        ...
        if (response.ok) {
          ...
          // Refresh the list
          const refreshResponse = await fetch('/api/operating-expenses/categories');
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                            <Link
                              href={`/office-management/operating-expenses/${category.id || category._id}`}
