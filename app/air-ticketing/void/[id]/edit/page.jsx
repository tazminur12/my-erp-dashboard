...
const EditVoid = ({ params }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch void record
        const voidRes = await fetch(`/api/air-ticketing/void/${id}`);
        if (voidRes.ok) {
          const vData = await voidRes.json();
          const record = vData.data;
          setFormData({
            ...
            voidCharge: record.voidCharge || 0,
            status: record.status || 'Pending',
            remarks: record.remarks || ''
          });
        } else {
          throw new Error('Failed to fetch void record');
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`/api/air-ticketing/void/${id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        Swal.fire({
          icon: 'success',
          title: 'সফল!',
          text: 'ভয়েড রেকর্ড সফলভাবে আপডেট হয়েছে',
          timer: 1500,
          showConfirmButton: false
        }).then(() => {
          router.push('/air-ticketing/void');
        ...
        throw new Error(data.error || 'Failed to update void record');
      }
    } catch (error) {
          ...
          <div className="mb-8 flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 mt-1">
                এয়ার টিকেট ভয়েড তথ্য পরিবর্তন করুন
              </p>
            </div>
            <Link
