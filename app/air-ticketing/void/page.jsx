...
const VoidList = () => {
  ...
  const fetchVoids = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/air-ticketing/void?${params.toString()}`);
      const result = await response.json();

      if (response.ok) {
        setVoids(result.data || []);
  ...
  const handleDelete = async (record) => {
    const result = await Swal.fire({
      ...
      text: `টিকেট নং "${record.ticketNumber}" এর ভয়েড রেকর্ড মুছে ফেলতে চান?`,
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`/api/air-ticketing/void/${record._id}`, {
          method: 'DELETE'
        });
        ...
        if (response.ok) {
          Swal.fire({
            title: 'সফল!',
            text: 'ভয়েড রেকর্ড সফলভাবে মুছে ফেলা হয়েছে',
            icon: 'success',
            confirmButtonText: 'ঠিক আছে',
        ...
        Swal.fire({
          title: 'ত্রুটি!',
          text: error.message || 'ভয়েড মুছতে সমস্যা হয়েছে',
          icon: 'error',
          confirmButtonText: 'ঠিক আছে',
          confirmButtonColor: '#EF4444'
        });
  ...
  const stats = useMemo(() => ({
    total: voids.length,
    pending: voids.filter(r => r.status === 'Pending').length,
    processed: voids.filter(r => r.status === 'Processed').length,
    totalAmount: voids.reduce((sum, r) => sum + (r.voidCharge || 0), 0)
  }), [voids]);
  ...
  return (
    <DashboardLayout>
      <div className="p-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 mt-2">
                এয়ার টিকেট ভয়েড ম্যানেজমেন্ট সিস্টেম
              </p>
            </div>
            <Link
              href="/air-ticketing/void/add"
              className="inline-flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-6 py-3 rounded-lg font-medium transition-colors shadow-lg"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600 dark:text-gray-400">মোট ভয়েড</p>
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600 dark:text-gray-400">মোট চার্জ</p>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-6">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="md:w-48">
              <div className="relative">
                ...
                <select
                  ...
                  className="w-full pl-10 pr-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent appearance-none bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                >
            ...
            <button
              ...
              title="রিফ্রেশ"
            >
              <Loader2 className={`w-5 h-5 text-gray-600 dark:text-gray-400 ${isLoading ? 'animate-spin' : ''}`} />
            </button>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">কোনো ভয়েড রেকর্ড পাওয়া যায়নি</h3>
              <p className="text-gray-500 dark:text-gray-400 mb-4">
                নতুন ভয়েড রেকর্ড তৈরি করতে উপরের বোতামে ক্লিক করুন
              </p>
            </div>
          ) : (
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <th className="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">যাত্রী</th>
                    <th className="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">ভেন্ডর</th>
                    <th className="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">ভয়েড চার্জ</th>
                  ...
                  {voids.map((record) => (
                    <tr key={record._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">
                        {new Date(record.voidDate).toLocaleDateString('bn-BD')}
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <span className="font-bold text-gray-900 dark:text-white">
                          ৳{(record.voidCharge || 0).toLocaleString()}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Link
                            href={`/air-ticketing/void/${record._id}/edit`}
        ...
        {/* Pagination Info */}
        {!isLoading && voids.length > 0 && (
          <div className="mt-6 text-center text-sm text-gray-500 dark:text-gray-400">
            মোট {pagination.total || voids.length} টি রেকর্ড দেখানো হচ্ছে
          </div>
        )}
      </div>
    </DashboardLayout>
