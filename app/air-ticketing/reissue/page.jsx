...
const ReissueList = () => {
  ...
  const fetchReissues = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/air-ticketing/reissue?${params.toString()}`);
      const result = await response.json();

      if (response.ok) {
        setReissues(result.data || []);
  ...
  const handleDelete = async (record) => {
    const result = await Swal.fire({
      ...
      text: `টিকেট নং "${record.ticketNumber}" এর রিইস্যু রেকর্ড মুছে ফেলতে চান?`,
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`/api/air-ticketing/reissue/${record._id}`, {
          method: 'DELETE'
        });
        ...
        if (response.ok) {
          Swal.fire({
            title: 'সফল!',
            text: 'রিইস্যু রেকর্ড সফলভাবে মুছে ফেলা হয়েছে',
            icon: 'success',
            confirmButtonText: 'ঠিক আছে',
        ...
        Swal.fire({
          title: 'ত্রুটি!',
          text: error.message || 'রিইস্যু মুছতে সমস্যা হয়েছে',
          icon: 'error',
          confirmButtonText: 'ঠিক আছে',
          confirmButtonColor: '#EF4444'
        });
  ...
  // Stats calculation
  const stats = useMemo(() => ({
    total: reissues.length,
    pending: reissues.filter(r => r.status === 'Pending').length,
    processed: reissues.filter(r => r.status === 'Processed').length,
    totalAmount: reissues.reduce((sum, r) => sum + (r.totalCharge || 0), 0)
  }), [reissues]);
      ...
      <div className="p-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 mt-2">
                এয়ার টিকেট রিইস্যু ম্যানেজমেন্ট সিস্টেম
              </p>
            </div>
            <Link
              href="/air-ticketing/reissue/add"
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600 dark:text-gray-400">মোট রিইস্যু</p>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">কোনো রিইস্যু রেকর্ড পাওয়া যায়নি</h3>
              <p className="text-gray-500 dark:text-gray-400 mb-4">
                নতুন রিইস্যু রেকর্ড তৈরি করতে উপরের বোতামে ক্লিক করুন
              </p>
            </div>
          ) : (
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <th className="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">মোট চার্জ</th>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {reissues.map((reissue) => (
                    <tr key={reissue._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">
                        {new Date(reissue.reissueDate).toLocaleDateString('bn-BD')}
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="font-medium text-gray-900 dark:text-white flex items-center gap-2">
                            <Ticket className="w-3 h-3 text-indigo-500" />
                            {reissue.ticketNumber}
                          </span>
                          {reissue.pnr && (
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="text-sm text-gray-900 dark:text-white flex items-center gap-2">
                            ...
                            {reissue.vendorName || 'N/A'}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="font-bold text-gray-900 dark:text-white">
                          ৳{reissue.totalCharge.toLocaleString()}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        {getStatusBadge(reissue.status)}
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Link
                            href={`/air-ticketing/reissue/${reissue._id}/edit`}
        ...
        {!isLoading && reissues.length > 0 && (
          <div className="mt-6 text-center text-sm text-gray-500 dark:text-gray-400">
            মোট {pagination.total || reissues.length} টি রেকর্ড দেখানো হচ্ছে
          </div>
        )}
      </div>
    </DashboardLayout>
