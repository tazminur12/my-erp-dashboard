...
const AddDilar = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/api/money-exchange/dilars', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        Swal.fire({
          title: 'সফল!',
          text: 'ডিলার সফলভাবে তৈরি করা হয়েছে',
          icon: 'success',
          confirmButtonText: 'ঠিক আছে',
          confirmButtonColor: '#7c3aed',
        });
        ...
        throw new Error(data.error || 'Failed to create dilar');
      }
    } catch (error) {
      console.error('Failed to create dilar:', error);
      Swal.fire({
        title: 'ত্রুটি!',
        text: error.message || 'ডিলার তৈরি করতে ব্যর্থ হয়েছে',
        icon: 'error',
        confirmButtonText: 'ঠিক আছে',
        confirmButtonColor: '#EF4444'
      });
      ...
      <div className="space-y-6 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-xl p-5 sm:p-6 md:p-8 border border-gray-200 dark:border-gray-700 shadow-sm">
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-end">
            ...
            >
              <Save className="w-4 h-4" /> {isSubmitting ? 'সংরক্ষণ হচ্ছে...' : 'ডিলার সংরক্ষণ করুন'}
            </button>
          </div>
        </form>
      </div>
