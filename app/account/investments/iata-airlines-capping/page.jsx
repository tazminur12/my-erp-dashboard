...
const IATAAirlinesCapping = () => {
  ...
  const fetchInvestments = async () => {
    ...
    try {
      ...
      const url = `/api/investments/iata-airlines-capping${queryParams.toString() ? `?${queryParams.toString()}` : ''}`;
  ...
  const handleAdd = () => {
    router.push('/account/investments/iata-airlines-capping/add');
  };

  const handleEdit = (item) => {
    const investmentId = item.id || item._id;
    router.push(`/account/investments/iata-airlines-capping/${investmentId}/edit`);
  };

  const handleView = (item) => {
    const investmentId = item.id || item._id;
    console.log('Navigating to investment details:', { item, investmentId });
    router.push(`/account/investments/iata-airlines-capping/${investmentId}`);
  };
