...
const AgentPackageEdit = () => {
  ...
  const packageId = params.id; // Package ID for editing
  ...
  const fetchPackageData = async () => {
    try {
      ...
      // Try /api/packages first
      let response = await fetch(`/api/packages/${packageId}`);
      let data = await response.json();
      
      // If not found, try /api/agent-packages
      if (!response.ok || (!data.package && !data.data)) {
        response = await fetch(`/api/agent-packages/${packageId}`);
      ...
      if (response.ok && (data.package || data.data)) {
        ...
        setFormData({
          ...
          sarToBdtRate: pkg.sarToBdtRate?.toString() || '',
          notes: pkg.notes || '',
          agentId: pkg.agentId?.toString() || pkg.agent_id?.toString() || '',
        ...
        if (pkg.totals?.discount !== undefined) {
          setDiscount(pkg.totals.discount.toString());
        } else if (pkg.discount !== undefined) {
          setDiscount(pkg.discount.toString());
        }
        
        // Pre-fill passenger arrays with IDs
        ...
        if (pkg.attachments) {
          setAttachments(pkg.attachments);
        }
      } else {
      ...
      Swal.fire({
        title: 'Error!',
        text: error.message || 'Failed to load package',
        icon: 'error',
        confirmButtonColor: '#EF4444',
      }).then(() => {
        router.push('/hajj-umrah/b2b-agent/agent-packages');
      });
    } finally {
      ...
      <DashboardLayout>
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400">প্যাকেজ লোড করা হচ্ছে...</p>
          </div>
        </div>
      </DashboardLayout>
    );
