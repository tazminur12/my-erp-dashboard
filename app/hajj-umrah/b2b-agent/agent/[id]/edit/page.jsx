...
const EditAgent = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (agent.profilePicture) {
          setProfilePreview(agent.profilePicture);
        }
      } else {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: error.message || 'Failed to load agent data',
  ...
  const handleReset = () => {
    if (originalForm) {
      ...
      setTouched({});
      setProfilePreview(originalForm.profilePicture || null);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Something went wrong while updating the agent');
      }
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: error.message || 'Something went wrong while updating the agent',
        confirmButtonColor: '#3b82f6',
      });
    } finally {
      setLoading(false);
    ...
    <DashboardLayout>
      <div className="space-y-6 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center space-x-3">
          <Link
            href={`/hajj-umrah/b2b-agent/agent/${id}`}
            ...
            <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-4">
              {profileUploading ? (
                ...
                <div className="flex flex-col items-center space-y-3">
                  ...
                  />
                  <div className="flex items-center space-x-3">
                    <label htmlFor="profile-picture-change" className="flex items-center space-x-2 px-3 py-1 text-sm text-blue-500 hover:text-blue-700 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors cursor-pointer">
