  ...
  const fetchPackageData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (pkg.totals && typeof pkg.totals === 'object') {
          setTotals({
            total: numberOrEmpty(pkg.totals.total),
            totalBD: numberOrEmpty(pkg.totals.totalBD || pkg.totals.grandTotal),
            grandTotal: numberOrEmpty(pkg.totals.grandTotal),
            passengerTotals: {
              adult: numberOrEmpty(pkg.totals.passengerTotals?.adult),
              child: numberOrEmpty(pkg.totals.passengerTotals?.child),
              infant: numberOrEmpty(pkg.totals.passengerTotals?.infant),
            },
          });
        }
      } else {
