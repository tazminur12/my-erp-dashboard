...
const UmrahPackageList = () => {
  ...
  const fetchUmrahCustomers = async () => {
    try {
      ...
      if (response.ok) {
        setUmrahCustomers(data.data || data.umrahs || []);
      } else {
        throw new Error(data.error || 'Failed to fetch umrah customers');
      }
    } catch (error) {
      console.error('Error fetching umrah customers:', error);
    } finally {
  ...
  const handleView = (pkg) => {
    router.push(`/hajj-umrah/package-list/${pkg._id}`);
  };

  const handleEdit = (pkg) => {
    router.push(`/umrah/umrah-package-list/${pkg._id}/edit`);
  };
  ...
  const { totalBookings, totalRevenue } = useMemo(() => {
    ...
    const umrahPackageIds = new Set(packages.map(pkg => pkg._id));

    // Filter customers that belong to Umrah packages
    umrahCustomers.forEach(customer => {
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        ...
        <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
          ...
          <button 
            onClick={() => router.push('/hajj-umrah/umrah/umrah-package-creation')}
