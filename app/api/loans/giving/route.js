  ...
  try {
    ...
    if (loansWithIds.length > 0) {
      loansWithIds.forEach(loan => {
        if (loan.loanId) {
          const idNumber = parseInt(loan.loanId.toUpperCase().replace(/^LN-?/, '')) || 0;
    ...
    const loanId = `LN${String(maxNumber + 1).padStart(6, '0')}`;
...
// GET all loans with search and filters
