...
export async function POST(request) {
  try {
    ...
    const newUmrah = {
      ...
      created_by: body.createdBy || userSession?.user?.id || userSession?.user?._id || userSession?.user?.email,
      employee_id: body.employeeId || userSession?.user?.employeeId,
      source_type: body.source_type || 'office',
      branch_id: body.branchId || body.branch_id || userSession?.user?.branchId || branchInfo.branchId || '',
