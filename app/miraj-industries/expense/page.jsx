...
const ExpenseList = () => {
  ...
  const fetchExpenses = async () => {
    try {
      setIsLoading(true);
      let url = '/api/miraj-industries/farm-expenses?';
      ...
      <div className="p-6 bg-gray-50 dark:bg-gray-900 min-h-screen space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">ব্যয় ব্যবস্থাপনা</h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">সকল খরচের তালিকা ও বিবরণ</p>
            </div>
          </div>
          <Link
            href="/miraj-industries/expense/add"
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="ক্যাটাগরি, বিবরণ বা ভেন্ডর খুঁজুন..."
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-transparent focus:ring-2 focus:ring-red-500 focus:border-transparent dark:text-white"
            />
          </div>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-900/50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">ক্যাটাগরি ও বিবরণ</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">ভেন্ডর</th>
                ...
                {isLoading ? (
                  ...
                  expenses.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right whitespace-nowrap text-sm font-bold text-red-600 dark:text-red-400">
                        ৳{Number(item.amount).toLocaleString('bn-BD')}
                      </td>
                      <td className="px-6 py-4 text-right whitespace-nowrap text-sm font-medium">
                        <div className="flex items-center justify-end gap-2">
                          <Link
                            href={`/miraj-industries/expense/${item.id}`}
