 ...
 */
export const searchFlights = async (searchRequest, options = {}) => {
  try {
    const token = await getSabreToken();
    const baseUrl = getBaseUrl();
    ...
    const doRequest = async (payload, reqTypeName = "50ITINS") => {
      payload.OTA_AirLowFareSearchRQ.TPA_Extensions.IntelliSellTransaction.RequestType.Name = reqTypeName;
    ...
    try {
      const data = await doRequest(requestPayload, "50ITINS");
      return data;
    } catch (e1) {
      const msg = e1.response?.data || e1.message;
      if (!options.quiet) console.error('Sabre Search Error[50ITINS]:', msg);
      // Fallback 1: try 200 itineraries
      try {
        const data2 = await doRequest(requestPayload, "200ITINS");
        return data2;
      } catch (e2) {
        if (!options.quiet) console.error('Sabre Search Error[200ITINS]:', e2.response?.data || e2.message);
        // Fallback 2: remove SplitTicketing extension and retry
        try {
          const stripped = JSON.parse(JSON.stringify(requestPayload));
          delete stripped.OTA_AirLowFareSearchRQ.TravelPreferences.TPA_Extensions.SplitTicketing;
          const data3 = await doRequest(stripped, "50ITINS");
          return data3;
        } catch (e3) {
          if (!options.quiet) console.error('Sabre Search Error[stripped]:', e3.response?.data || e3.message);
          throw e1.response?.data || e1;
        }
      }
    }
  } catch (error) {
    if (!options.quiet) console.error('Sabre Search Error:', error.response?.data || error.message);
