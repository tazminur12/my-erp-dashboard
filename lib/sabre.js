...
export const searchFlights = async (searchRequest) => {
  try {
    ...
    try {
      ...
      console.error('Sabre Search Error[50ITINS]:', msg);
      // Fallback 1: try 200 itineraries
      try {
        const data2 = await doRequest(requestPayload, "200ITINS");
        return data2;
      } catch (e2) {
        console.error('Sabre Search Error[200ITINS]:', e2.response?.data || e2.message);
        ...
        try {
          ...
          console.error('Sabre Search Error[stripped]:', e3.response?.data || e3.message);
